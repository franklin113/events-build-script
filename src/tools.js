...
function formatForVmeet(renderedHtml, $) {
  const vmeetFormatted = `</style>${renderedHtml}<style>`;
  return vmeetFormatted;
}
...
function formatHtml(config) {
  ...
  // * render the html
  let renderedHtml = $('body').html();
  if (renderedHtml.match(/\*\*VMEET\*\*/g)) {
    renderedHtml = renderedHtml.replace(/\/\/.*\*\*VMEET\*\*/g, '');
    renderedHtml = formatForVmeet(renderedHtml);
  }
